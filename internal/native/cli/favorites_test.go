...
func TestFavoritesOpenByTitleCaseInsensitive(t *testing.T) {
	flags := &rootFlags{Name: "Kitchen", Timeout: 2 * time.Second}
	cmd := newFavoritesOpenCmd(flags)
	...
	fake := &fakeFavoritesClient{
		page: sonos.FavoritesPage{
			Items: []sonos.FavoriteItem{
				{Position: 1, Item: sonos.DIDLItem{Title: "Apple Music Mix", URI: "x://amix"}},
				{Position: 2, Item: sonos.DIDLItem{Title: "Jazz Chill", URI: "x://jazz"}},
			},
			NumberReturned: 2,
			TotalMatches:   2,
		},
	...
	cmd.SetOut(newDiscardWriter())
	cmd.SetErr(newDiscardWriter())
	cmd.SetArgs([]string{"apple music mix"})
	cmd.SilenceErrors = true
	cmd.SilenceUsage = true
	if err := cmd.ExecuteContext(context.Background()); err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if fake.lastItem.Title != "Apple Music Mix" {
		t.Fatalf("expected Apple Music Mix, got %q", fake.lastItem.Title)
	}
}
