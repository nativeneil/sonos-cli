...
func (c Config) Normalize() Config {
	...
	if out.Timeout != "" {
		if _, err := time.ParseDuration(out.Timeout); err != nil {
			out.Timeout = ""
		}
	}
	return out
...
func NewDefaultStore() (*FileStore, error) {
	...
	return &FileStore{path: filepath.Join(dir, "sonos-playlist", "config.json")}, nil
}
...
func (s *FileStore) Load() (Config, error) {
	...
	// Backward compatibility: older configs may store keys at top level.
	if ff.Config == (Config{}) {
		var legacy Config
		if err := json.Unmarshal(raw, &legacy); err == nil && legacy != (Config{}) {
